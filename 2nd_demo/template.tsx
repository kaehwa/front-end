...
export default function Recommendations() {
  ...
  useEffect(() => {
    (async () => {
      try {
        // 실제 API 예:
        // const res = await fetch(`${BACKEND_URL}/flowers/{}/similar;);
  ...
  let binary = '';
  const chunk = 0x8000; // 퍼포먼스 최적화
  for (let i = 0; i < byteArray.length; i += chunk) {
    const sub = byteArray.slice(i, i + chunk);
  ...
  try {
    const res = await fetch(
      `${BACKEND_URL}/flowers/${ID}/recommendations?giver=${encodeURIComponent(meta.giver)}&receiver=${encodeURIComponent(meta.receiver)}&occasion=${encodeURIComponent(meta.occasion)}`
    );

    if (!res.ok) throw new Error("네트워크 응답 오류");

    // 백엔드에서 Bouquet[] + byte[]
    const remote: { id: string; title: string; image: number[]; tags?: string[]; palette?: string[] }[] =
      await res.json();

    const data: Bouquet[] = remote.map((item) => ({
      id: item.id,
      title: item.title,
      imageUrl: byteArrayToBase64(item.image), // 변환된 data URI
      tags: item.tags,
