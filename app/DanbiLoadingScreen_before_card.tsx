...
useEffect(() => {
  ...
  // 2️⃣ GET: 메시지 완료 여부 확인
  const checkMessage = async () => {
    try {
      const res = await fetch(`${BACKEND_URL}/flowers/${orderID}/medialetter`);
      console.log("checkMessage res")
      console.log(res)
      const data = await res.json();
  ...
  // 3️⃣ 루프: 메시지 완료될 때까지 polling
const loop = async () => {
  const start = Date.now(); // 시작 시간 기록

  while (active && !isReady) {
    const msg = await checkMessage();
    if (!active) break;

    if (msg?.voiceletterBase64) {
      console.log("✅ DB에서 메시지 확인됨:");
      setIsReady(true);
      setProgress(1);
    ...
    // 타임아웃 체크 (3분 = 180,000ms)
    if (Date.now() - start > 180000) {
      console.warn("⏰ 3분 지남 → 강제 이동");
      router.push({
        pathname: "/card",
        params: { orderID: orderID ?? "" },
      });
    ...
    await new Promise((r) => setTimeout(r, 10000)); // 10초마다 재시도
  }
};
  loop()
