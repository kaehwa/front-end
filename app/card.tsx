...
const BACKEND_URL = "http://4.240.103.29:8080"//"http://4.240.103.29:8080";
...
export default function CardScreen() {
  ...
  async function base64ToUri(base64: string, fileName: string): Promise<string> {
    if (Platform.OS === "web") {
      // 1. base64 → Blob 변환
      const byteCharacters = atob(base64);
      const byteNumbers = new Array(byteCharacters.length).fill(0).map((_, i) => byteCharacters.charCodeAt(i));
      const byteArray = new Uint8Array(byteNumbers);
      const blob = new Blob([byteArray], { type: "audio/wav" }); // wav라면 "audio/wav"

      // 2. Blob → ObjectURL
      return URL.createObjectURL(blob); // web 전용 경로
    } else {
      // 3. 네이티브 환경에서는 기존 로직 그대로
      const uri = `${FileSystem.documentDirectory}${fileName}`;
      console.log("uri trank")
      console.log(uri)
  ...
  const fetchCard = useCallback(async (cardId: string) => {
    ...
    try {
      ...
      try {
        ...
        console.log("track base64Audio")
        console.log(typeof base64Audio)
        if (base64Audio && typeof base64Audio === "string") {
          console.log("voiceletterBase64")
          //audioUri = await writeB64(base64Audio, `audio_${String(cardId)}.m4a`);
          audioUri = await base64ToUri(base64Audio, `audio_${String(cardId)}.wav`);

          console.log("audioUri if clause")
